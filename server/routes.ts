...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get orders pending approval for a PM
  apiRouter.get("/pm/orders/pending", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const pendingOrders = await storage.getOrdersForApproval(pmId);
  ...
  // Get approved orders by the PM
  apiRouter.get("/pm/orders/approved", async (req: Request, res: Response) => {
    if (!req.isAuthenticated() || req.user?.role !== 'project_manager') {
      return res.status(403).json({ message: "Not authorized. Project Manager role required." });
    }
    ...
    try {
      ...
      const approvedOrders = await storage.getApprovedOrdersByPM(pmId, limit);
      
      // Get full details for each order
      const ordersWithDetails = await Promise.all(
        approvedOrders.map(async order => {
          const items = await storage.getOrderItems(order.id);
          const business = await storage.getBusiness(order.businessId);
  ...
  // Approve an order
  apiRouter.post("/orders/:id/approve", async (req: Request, res: Response) => {
    if (!req.isAuthenticated() || req.user?.role !== 'project_manager') {
      return res.status(403).json({ message: "Not authorized. Project Manager role required." });
    }
    ...
    try {
      ...
      const updatedOrder = await storage.approveOrder(orderId, pmId, notes || "");
      
      // Get full details for the order
      const items = await storage.getOrderItems(updatedOrder.id);
      const business = await storage.getBusiness(updatedOrder.businessId);
      ...
      res.json({ ...updatedOrder, business, items: itemsWithDetails });
    } catch (error) {
  ...
  // Reject an order
  apiRouter.post("/orders/:id/reject", async (req: Request, res: Response) => {
    if (!req.isAuthenticated() || req.user?.role !== 'project_manager') {
      return res.status(403).json({ message: "Not authorized. Project Manager role required." });
    }
    ...
    try {
      ...
      const updatedOrder = await storage.rejectOrder(orderId, pmId, reason);
  ...
  // Modify an order
  apiRouter.post("/orders/:id/modify", async (req: Request, res: Response) => {
    if (!req.isAuthenticated() || req.user?.role !== 'project_manager') {
      return res.status(403).json({ message: "Not authorized. Project Manager role required." });
    }
    ...
    try {
      ...
      const updatedOrder = await storage.modifyOrder(orderId, pmId, items, notes || "");
